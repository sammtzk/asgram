...
def _dsdsc(
        y, zar, src_area=None,
        mu=1/3, dpi=72, cross_eyed=False, approach='rl', hsr=True
):
    ...
    for x in scan_order:
        s = _pix_sep(zar[x, y], mu, dpi, cross_eyed)
        left = x - round(s / 2)
        right = left + s

        if 0 <= left and right < w:
            if hsr:  # hidden surface removal
...
class PixCon:
    ...
    def __init__(self, zmap: ZMap, p: Params, update_on_init=True):
        self.zmap = zmap
        self.p = p
    ...
    def src_area_estimate(self):
        ...
        if (
            (self.p.approach not in ['mo', 'oi', 'lr', 'rl'])
            or (not self.p.fill)
        ):
            return ~src_area

        for y in range(height):
            _z = np.max(zm_arr[:, y]) if self.p.cross else np.min(zm_arr[:, y])
            max_sep = _pix_sep(_z, self.p.mu, self.p.dpi, self.p.cross)

            if 'oi' != self.p.approach:
                match self.p.approach:
                    case 'mo':
    ...
    def update(self):
        ...
        jobs = worker_count(self.p.num_jobs)
        if 1 < jobs:
